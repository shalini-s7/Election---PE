...
export default function Module() {
  ...
  const { completedModules, moduleProgress, completeModule, setModuleProgress, simpleLanguage } = useProgress();
  ...
  const simplifyText = (text) => {
    ...
    return text
      .replace(/eligibility/g, 'rules to vote')
      .replace(/residency requirements/g, 'rules about where you live')
      .replace(/designated agencies/g, 'official places')
      .replace(/affiliation/g, 'choice of group')
      .replace(/jurisdiction/g, 'local area')
      .replace(/canvassing/g, 'checking the votes')
      .replace(/certification/g, 'making it official')
      .replace(/audits/g, 'double-checks');
  };
      ...
      <div className="container module-detail">
        ...
        <div className="card module-step animate-in" style={{ borderLeftColor: 'var(--primary)' }}>
          <h3 style={{ fontSize: '1.2rem', marginBottom: 12 }}>
            ...
            {step.title}
            <WhyTooltip topic={step.title} />
          </h3>
          <p style={{ lineHeight: 1.8, color: 'var(--text)' }}>
            {simplifyText(step.content)}
          </p>
          {step.fact && (
